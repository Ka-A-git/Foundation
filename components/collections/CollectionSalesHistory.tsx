...
export default function CollectionSalesHistoryTable(
          ...
          () => [
            {
              Header: 'Artwork',
              accessor: 'artwork',
              Cell: ({ value }) => <ArtworkTableCell artwork={value} />,
  ...
  useEffect(() => {
    setHiddenColumns(isMobile ? ['eventType', 'buyer', 'seller'] : []);
...
function UserTableCell(props: UserTableCellProps) {
  ...
  return (
    <Box css={{ position: 'relative', zIndex: 2 }}>
      {user && <UserTag user={user} appearance="plain" />}
    </Box>
  );
}
