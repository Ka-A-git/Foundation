...
import { getFirstValue, isEmptyOrNil } from 'utils/helpers';
import {
  createModelMimeType,
...
export default function UploadMedia(): JSX.Element {
  ...
  const handleUpload = useCallback(
    async (
      ...
      const { file, contractAddress } = values;

      const pinataApiKey = await generatePinataApiKey();
  ...
  return (
    <Formik<UploadMediaFormValues>
      ...
      enableReinitialize
      initialValues={{ file: null, modelPoster: null, contractAddress }}
    >
      {(formikContext) => (
        <Form
          style={{
