...
export const parseDateToUnix: (arg0: string | Date) => number = when(
  notEmptyOrNil,
...
export const parseAlgoliaTimestamp = (timestamp: string | number) => {
  // if timestamp is UNIX, convert to milliseconds and to ISO string
  const newTimestamp =
    typeof timestamp === 'number'
      ? new Date(timestamp * 1000).toISOString()
      : timestamp;
  return parseDateToUnix(newTimestamp);
};
