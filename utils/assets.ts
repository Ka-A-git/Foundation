...
export const buildFallbackCoverImageUrl = compose<string, string>(
  whenURLIsValid(
    buildImgixUrlNew({
      q: 75,
      w: 1600,
...
export const getCreatorCardHero = compose<UserLight, string, string>(
  whenURLIsValid(
    buildImgixUrlNew({
      q: 30,
      'min-w': 400,
...
const buildModelPosterImageUrl = (
  ...
  if (artwork.assetVersion !== 5 && artwork.assetId) {
  ...
  // otherwise we check for precense of assetId and fall-back to assetpath
  const assetCid = artwork.assetId || getIpfsHash(artwork.assetPath);
  const path = buildS3FolderStructure(assetCid);
  const url = `${modelImageAssetsHost}${path}/nft.png`;
...
export const buildArtworkModelUrl = (artwork: Artwork): string => {
  return cond<Artwork, string>([
    ...
    [
      T,
      (artwork: Artwork) =>
        `${artwork.assetScheme}${artwork.assetHost}/ipfs/${artwork.assetIPFSPath}`,
    ],
  ])(artwork);
};
...
export const buildActivityCardFallbackAssetUrl = compose<string, string>(
  whenURLIsValid(
    buildImgixUrlNew({
      h: 260,
...
export const buildArtworkCardFallbackAssetUrl = compose<string, string>(
  whenURLIsValid(
    buildImgixUrlNew({
      ...cardAssetDefaults,
